...
contract NonFungiblePositionManagerBatchTest is TokenFixture, Test, GasSnapshot {
    ...
    function testBatchMint() external {
...

        {
            ...
            data[1] = abi.encode(
                INonfungiblePositionManager.CallbackData(
                    address(this), INonfungiblePositionManager.CallbackDataType.CLOSE_CURRENCY, abi.encode(currency0)
                )
            );
            data[2] = abi.encode(
                INonfungiblePositionManager.CallbackData(
                    address(this), INonfungiblePositionManager.CallbackDataType.CLOSE_CURRENCY, abi.encode(currency1)
                )
            );
